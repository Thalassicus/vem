    ...
    if (not Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
 		for tech in GameInfo.Technologies() do
 			...
 			if pPlayer:CanResearch( techID ) then
				AddTechButton( tech, pPlayer:GetYieldTurns(YieldTypes.YIELD_SCIENCE,  techID), iDiscover);
			end
		end
	end
...
function AddTechButton( tech, turnsLeft, iDiscover)
	...
	thisTechButtonInstance.FreeTechName:SetText( techName );
  	...
  	local researchTurnsLeft = math.ceil(pPlayer:GetYieldTurns(YieldTypes.YIELD_SCIENCE, tech.ID));
	...
	if pPlayer:GetCurrentResearch() == tech.ID then -- the pPlayer is currently researching this one
		...
		if iDiscover > 0 then
			...
			-- update number of turns to research
 			if 	pPlayer:GetYieldRate(YieldTypes.YIELD_SCIENCE) > 0 then
		...
		local researchTurnsLeft = pPlayer:GetYieldTurns(YieldTypes.YIELD_SCIENCE,  techID);
		local currentResearchProgress = teamTechs:GetResearchProgress(techID) + pPlayer:GetOverflowResearch();
		local researchNeeded = pPlayer:GetYieldNeeded(YieldTypes.YIELD_SCIENCE);
		local researchPerTurn = pPlayer:GetYieldRate(YieldTypes.YIELD_SCIENCE);
