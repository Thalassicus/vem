...
--		UpdateSpecialists(pCity);
		...
		-- display gold income
		local iGoldPerTurn = Game.Round(City_GetYieldRate(pCity, YieldTypes.YIELD_GOLD), 1);
		...
		if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_SCIENCE)) then
			...
			local iSciencePerTurn = Game.Round(City_GetYieldRate(pCity, YieldTypes.YIELD_SCIENCE), 1);
		...
		local iFoodPerTurn = Game.Round(City_GetYieldRateTimes100(pCity, YieldTypes.YIELD_FOOD) / 100,1);
...
function DoUpdateProductionInfo( bNoProduction )
	...
	local iProductionPerTurn = Game.Round(City_GetYieldRate(pCity, YieldTypes.YIELD_PRODUCTION), 1)
	...
	if (UI.IsCityScreenUp()) then   
		...
		if UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT then
			...
			for plotID, plot in Plots() do
				if Map.PlotDistance(pCity:Plot():GetX(), pCity:Plot():GetY(), plot:GetX(), plot:GetY()) <= GameDefines.MAXIMUM_BUY_PLOT_DISTANCE then
					local hexPos = ToHexFromGrid( Vector2( plot:GetX(), plot:GetY() ) );
					local worldPos = HexToWorld( hexPos );
					local purchaseCity, purchaseCost = City_GetBestPlotPurchaseCity(pCity, plot)
...
function OnMouseOverHex( hexX, hexY )
	...
	if UI.GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT then
		...
		if (pCity:CanBuyPlotAt( hexX, hexY, true)) then
			local iPlotCost = Plot_GetCost(pCity, Map.GetPlot( hexX, hexY ));
