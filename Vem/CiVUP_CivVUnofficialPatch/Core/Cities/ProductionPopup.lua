...
function ProductionSelected( eOrder, iData)
    ...
    if g_IsProductionMode then
		...
		elseif (eOrder == OrderTypes.ORDER_CONSTRUCT) then
			...
			local costDiff = city:GetBuildingPurchaseCost(iData) - City_GetPurchaseCost(city, GameInfo.Buildings, iData)
			if costDiff ~= 0 then
 	...
 	for k,v in ipairs(productionList.Units) do
		...
		--print(v.Name.." "..v.Priority.." "..v.TechCost);
 		if g_IsProductionMode then
			...
			if City_IsPurchaseable(city, true, unitID, -1, -1) then
 				local isDisabled = false;
     			-- test w/o visible (ie can train right now)
	     	
    			if (not City_IsPurchaseable(city, false, unitID, -1, -1)) then
    				isDisabled = true;
				end
	 			
 				local cost = City_GetPurchaseCost(city, GameInfo.Units, unitID)
 	 	...
 	 	if g_IsProductionMode then
			...
			if city:IsCanPurchase(true, -1, -1, projectID) then
				local isDisabled = true;		    
 				local cost = City_GetPurchaseCost(city, GameInfo.Projects, projectID)
    ...
    -- Buildings	
 	for k,v in ipairs(productionList.Buildings) do
		...
		if prereq.Info then
 		...
 		if g_IsProductionMode then
 			-- test w/ visible (ie COULD train if ... )
			if City_IsBuildable(city, buildingID, 0, 1 ) then
				local isDisabled = false;
			    
     			-- test w/o visible (ie can train right now)
    			if not City_IsBuildable(city, buildingID) or prereq.BuildingsNeeded ~= 0 then
    				isDisabled = true;
				end
			...
			if City_IsPurchaseable(city, true, -1, buildingID, -1) then
				local col = 2;
				local thisBuildingClass = GameInfo.BuildingClasses[building.BuildingClass];
				if thisBuildingClass.MaxGlobalInstances > 0 or thisBuildingClass.MaxPlayerInstances > 0 or thisBuildingClass.MaxTeamInstances > 0 then
					col = 3;
 				...
 				local isDisabled = false;
    			if not City_IsPurchaseable(city, false, -1, buildingID, -1) then
 					isDisabled = true;
 				end
	 			
 				local cost = City_GetPurchaseCost(city, GameInfo.Buildings, buildingID)
