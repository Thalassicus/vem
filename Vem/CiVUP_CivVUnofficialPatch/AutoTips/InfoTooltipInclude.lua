...
if true then--MapModData.Fields.Buildings == nil then
...
function GetYieldTooltip(city, yieldType)
	...
	local iYieldFromTerrain = Game.Round(City_GetBaseYieldFromTerrain(city, yieldType));
	...
	local iYieldFromBuildings = Game.Round(City_GetBaseYieldFromBuildings(city, yieldType));
	...
	local iYieldFromSpecialists = Game.Round(City_GetBaseYieldFromSpecialists(city, yieldType));
	...
	-- Base Yield from Pop
	local iYieldFromPop = Game.Round(City_GetBaseYieldFromPopulation(city, yieldType));
	...
	local iYieldFromPolicies = Game.Round(City_GetBaseYieldFromPolicies(city, yieldType));
	...
	local iYieldFromTraits = Game.Round(City_GetBaseYieldFromTraits(city, yieldType));
	...
	local iYieldFromProcesses = Game.Round(City_GetBaseYieldFromProcesses(city, yieldType));
	...
	local iYieldFromMisc = Game.Round(City_GetBaseYieldFromMisc(city, yieldType));
	...
	if Civup.ENABLE_DISTRIBUTED_MINOR_CIV_YIELDS then
		local playerMinorCivYield	= owner:GetYieldsFromCitystates()[yieldType];
		if playerMinorCivYield > 0 then
			local cityWeight		= City_GetWeight(city, yieldType);
			local playerWeight		= owner:GetTotalWeight(yieldType);
	...
	local surplusModFromGAs = owner:GetGoldenAgeSurplusYieldModifier(yieldType)
	...
	local iYieldFromOtherYields = Game.Round(City_GetYieldFromFood(city, yieldType));
